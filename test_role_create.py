...
def test_update_role_level_out_of_range_max(role, book , base_url):
...
def test_create_role_without_name(book , base_url):
    role_data = { "type": "Drama","level": 2 , "book"  : book["id"]}
    resp = requests.post(f"{base_url}/roles", data = role_data)
    assert resp.status_code == 400
    print(resp.json())

def test_create_role_without_id(book , base_url):
    role_data = { "name": "Unknown", "type": "Drama","level": 2 , "book"  : book["id"]+1  }
    resp = requests.post(f"{base_url}/roles", data = role_data)
    assert resp.status_code == 400
    print(resp.json())
